...
INTEGRATIONS_DIR = Path(
    os.getenv("INTEGRATIONS_DIR", "/opt/homelab-dashboard/integrations/devices")
)
...
async def run_via_cli(prompt: str, sandbox: Path) -> Optional[str]:
    ...
    try:
        ...
        if proc.returncode != 0:
            log.warning(
                "claude CLI exited %d: %s", proc.returncode, stderr.decode()[:300]
            )
            return None
...
def _extract_code_block(text: str) -> str:
    ...
    code_lines = [
        ln
        for ln in lines
        if ln.startswith(("import ", "from ", "async def ", "def ", "DEVICE_", "#"))
    ]
...
async def run_device_analysis(device_context: dict) -> dict:
    ...
    try:
        ...
        output = await run_via_cli(prompt, sandbox) or await run_via_sdk(
            prompt, sandbox
        )
...
async def apply_change(
    change_id: str, sandbox_dir: str, generated_files: list[str]
) -> list[str]:
    """
